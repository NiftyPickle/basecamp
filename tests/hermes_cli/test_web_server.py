...
class TestPtyChatGracefulDegradation:
    """`/api/pty` must degrade gracefully when the Node/ink ``ui-tui`` bundle
    is absent (e.g. the desktop app stages CPython + web_dist only, no Node).

    ``_resolve_chat_argv`` -> ``_make_tui_argv`` does Path operations on the
    missing ``ui-tui`` dir and raises ``FileNotFoundError`` *before* the
    ``sys.exit(1)`` that the existing ``except SystemExit`` handler covers.
    The handler must catch it and send a friendly xterm message + close
    1011, not let it bubble to a 500 ASGI exception.
    """
    ...
    def _setup(self, monkeypatch, _isolate_hermes_home):
        ...
        # Force the spawn path: embedded chat on, PTY bridge present, and
        # every pre-accept gate (auth/host/client) waved through so the test
        # exercises only the argv-resolution guard.
    ...
    def test_missing_ui_tui_bundle_closes_cleanly(self):
        """A FileNotFoundError from argv resolution => friendly message + 1011,
        never an unhandled 500."""
        from starlette.websockets import WebSocketDisconnect

        def _boom(*_a, **_k):
            raise FileNotFoundError(
                2, "No such file or directory", "/Resources/.../site-packages/ui-tui"
            )

        self.monkeypatch.setattr(self.web_server, "_resolve_chat_argv", _boom)

        with self.client.websocket_connect("/api/pty") as ws:
            banner = ws.receive_text()
            assert "Chat" in banner
            # The path string must not leak verbatim; message stays generic.
            with pytest.raises(WebSocketDisconnect) as exc:
                ws.receive_text()
