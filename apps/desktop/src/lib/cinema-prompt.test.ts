...
describe("cinema catalogs", () => {
  ...
  it("cinemaAssetUrl resolves bundled tiles relative to the Vite base", () => {
    const url = cinemaAssetUrl("Full-Frame Cine Digital");
    expect(url).toMatch(/cinema\/full_frame_cine_digital\.webp$/);
    expect(url).toBe(`${import.meta.env.BASE_URL}cinema/full_frame_cine_digital.webp`);
  });

  it("cinemaAssetUrl returns null for labels without a tile", () => {
    expect(cinemaAssetUrl("35")).toBeNull();
    expect(cinemaAssetUrl("nonexistent")).toBeNull();
  });
});
