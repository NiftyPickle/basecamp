...
// Preview tile filenames under apps/desktop/public/cinema/ (copied from
// web/public/cinema/, themselves from Anil-matcha/Open-Generative-AI via the
// beyond app, MIT). Cameras, lenses, and apertures have tiles;
// focals/resolutions/aspects render as text chips.
...
// Desktop deviation from the web original: web served these tiles from the
// backend at `${HERMES_BASE_PATH}/cinema/*`. The Electron renderer has no
// same-origin backend, so the tiles are bundled into the renderer's public/
// dir and referenced relative to the Vite base (mirrors the assetPath helper
// in brand-mark.tsx).
export function cinemaAssetUrl(label: string): string | null {
  const file = CINEMA_ASSET_FILES[label];
  return file ? `${import.meta.env.BASE_URL}cinema/${file}` : null;
}
