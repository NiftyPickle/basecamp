...
import axios from "../vibe-axios.js";
...
// next/link not available in Electron; replaced with plain anchor (see usage below)
...
const NodeFlow = ({ initialNodeSchemas, initialWorkflowData }) => {
  ...
  const handleDuplicateWorkflow = async () => {
    ...
    try {
      const response = await axios.post("/api/workflow/create", workflowPayload);
      console.log("Workflow created:", response.data);
      // Electron renderer has no per-workflow URL; route back to the flow
      // canvas via react-router instead of a full-page reload to a path that
      // would 404 under file:// (was: window.location.href = `/workflow/${id}`).
      navigate("/flow");
    } catch (error) {
      console.log(error);
      setIsRunning(0);
      ...
      <div className="flex items-center justify-center absolute top-0 z-20 bg-[#151618] w-full py-3 border-b border-gray-800">
        <div className="flex items-center justify-between w-full max-w-[95%] sm:max-w-[90%] lg:max-w-[80%] overflow-x-auto">
          <div className="flex items-center gap-2 w-[35%]">
            {/* next/link replaced with plain anchor for Electron renderer */}
            <a
              href="/workflow"
