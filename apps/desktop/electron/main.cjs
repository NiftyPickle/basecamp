...
function resolveHermesCwd() {
  ...
  // `…/win-unpacked` on Windows or `/Applications/Basecamp.app/Contents/...`
...
function resolveHermesBackend(dashboardArgs) {
  ...
  if (process.env.HERMES_DESKTOP_IGNORE_EXISTING !== '1') {
    ...
    if (hermesCommand) {
      if (looksLikeDesktopAppBinary(hermesCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Basecamp CLI: ${hermesCommand}`)
        hermesCommand = null
      }
    }
    ...
    if (hermesCommand) {
      ...
      if (verifyHermesCli(hermesCommand, { shell: shellForProbe })) {
        return {
          label: `existing Basecamp CLI at ${hermesCommand}`,
      ...
      rememberLog(
        `Ignoring existing Basecamp CLI at ${hermesCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }
  ...
  return {
    ...
    label: 'Basecamp Agent not installed yet; bootstrap required',
    command: null,
    args: dashboardArgs,
    bootstrap: true,
    env: {},
...
// Studio reference upload is the one request that is NOT JSON: FastAPI's
// ``/api/studio/upload`` expects ``multipart/form-data`` with a single ``file``
// field (``UploadFile = File(...)``). A File/FormData cannot cross the IPC
// bridge intact, so the renderer (studio-api.ts uploadReference) sends a plain
// ``{ upload: { filename, contentType, bytes } }`` (bytes is a Uint8Array,
// which survives structured clone). Here we reconstruct the multipart envelope
// and POST it with the same auth attach-points as ``fetchJson`` /
// ``fetchJsonViaOauthSession``.
function buildStudioMultipart(upload) {
  const boundary = `----HermesStudioUpload${crypto.randomBytes(16).toString('hex')}`
  const filename = String(upload?.filename || 'upload.bin').replace(/["\r\n]/g, '')
  const contentType = String(upload?.contentType || 'application/octet-stream')
  const fileBuf = Buffer.from(upload?.bytes ?? [])
...
function parseBackendJson(url, statusCode, text, reject, resolve) {
  if ((statusCode || 500) >= 400) {
    reject(new Error(`${statusCode}: ${text || ''}`))
    return
  }
  if (!text) {
    resolve(null)
...
function fetchMultipart(url, connection, options = {}) {
  const { body, contentType } = buildStudioMultipart(options.upload)
  const timeoutMs = resolveTimeoutMs(options.timeoutMs, DEFAULT_FETCH_TIMEOUT_MS)

  // OAuth profiles authenticate via the HttpOnly cookie in the OAuth partition,
  // so route through Electron's net stack bound to that session (mirrors
  // fetchJsonViaOauthSession). Token/local profiles use the static header.
  if (connection.authMode === 'oauth') {
    return new Promise((resolve, reject) => {
      ...
      const request = electronNet.request({ method: 'POST', url, session: sess, useSessionCookies: true, redirect: 'follow' })
      ...
      request.on('response', res => {
        ...
        res.on('end', () => {
          if (timedOut) return
          clearTimeout(timer)
          parseBackendJson(url, res.statusCode || 500, Buffer.concat(chunks).toString('utf8'), reject, resolve)
        })
      })
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(
      ...
      res => {
        ...
        res.on('error', reject)
        res.on('data', chunk => chunks.push(chunk))
        res.on('end', () => parseBackendJson(url, res.statusCode || 500, Buffer.concat(chunks).toString('utf8'), reject, resolve))
      }
    )
    req.on('error', reject)
    req.setTimeout(timeoutMs, () => {
...
//
// Hosted Basecamp gateways gate the dashboard behind an OAuth provider (e.g.
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  // connect to Basecamp gateway". Mirror the renderer's connect here so the test
...
async function spawnPoolBackend(profile, entry) {
  ...
  child.once('exit', (code, signal) => {
    ...
    backendPool.delete(profile)
    if (!ready) {
      rejectStart?.(new Error(`Basecamp backend for profile "${profile}" exited before it became ready (${signal || code}).`))
    }
  })
...
ipcMain.handle('hermes:api', async (_event, request) => {
  ...
  // Multipart escape hatch: a body shaped `{ upload: { filename, contentType,
  // bytes } }` is a file upload (studio reference image). JSON serialization
  // would mangle the bytes, so reconstruct multipart/form-data instead.
  if (request?.body && typeof request.body === 'object' && request.body.upload) {
    return fetchMultipart(url, connection, { upload: request.body.upload, timeoutMs })
  }
