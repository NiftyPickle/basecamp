...
function RootRedirect() {
  return <Navigate to="/sidekick" replace />;
}
...
const SIDEKICK_NAV_ITEM: NavItem = {
  path: "/sidekick",
  labelKey: "sidekick",
  label: "Chat",
  icon: MessageSquare,
...
const BUILTIN_ROUTES_CORE: Record<string, ComponentType> = {
  "/": RootRedirect,
  "/sidekick": SimpleChatPage,
  "/studio": StudioPage,
  "/free-chat": FreeChatPage,
...
/** Built-in nav paths kept at the top level of the sidebar rather than tucked
 * under the "Advanced" accordion. Primary product surfaces only. */
const TOP_NAV_PATHS = new Set<string>(["/sidekick", "/studio", "/free-chat", "/organize"]);
...
export default function App() {
  ...
  // The Terminal "/chat" surface spawns the Node/ink `ui-tui` over a PTY
  // (/api/pty). The desktop bundle stages CPython + web_dist only — no Node
  // — so that spawn can't work there and the backend now closes the socket
  // with a "not bundled" notice. Hide the whole surface (nav item, route
  // sink, and the persistent PTY host that spawns on load) inside the
  // desktop shell so users never hit the broken tab; /sidekick (in-process
  // over /api/ws) remains the desktop chat surface. Mirrors the /organize
  // desktop gate below.
  const embeddedChat =
    isDashboardEmbeddedChatEnabled() && window.__HERMES_DESKTOP__ !== true;
  ...
  const builtinNav = useMemo(() => {
    // The organizer touches the local filesystem, so its nav entry only
    // appears inside the desktop shell.
    const navRest = BUILTIN_NAV_REST.filter(
      (item) => item.path !== "/organize" || window.__HERMES_DESKTOP__ === true,
    );
    const base = embeddedChat
      ? [SIDEKICK_NAV_ITEM, CHAT_NAV_ITEM, ...navRest]
      : [SIDEKICK_NAV_ITEM, ...navRest];
          ...
          >
            ...
            >
              <ul className="flex flex-col">
                {/* Promote primary product surfaces (Chat, Studio) to the top
                    level by path; everything else stays under "Advanced".
                    Path-based, not index-based, so it is robust to embedded-chat
                    inserting an extra item ahead of Studio. */}
                {sidebarNav.coreItems
                  .filter((item) => TOP_NAV_PATHS.has(item.path))
                  .map((item) => (
                    <SidebarNavLink
                ...
                {sidebarNav.coreItems.some((item) => !TOP_NAV_PATHS.has(item.path)) && (
                  <li>
                    <details className="group mt-1">
                      <summary
                        data-tour="advanced"
                        className={cn(
                          "cursor-pointer list-none px-5 py-2 text-xs uppercase tracking-[0.12em] text-text-tertiary hover:text-text-primary",
  ...
  return (
    ...
    >
      <NavLink
        ...
        data-tour={path === "/env" ? "keys" : path === "/sessions" ? "sessions" : undefined}
        onClick={closeMobile}
