...
declare global {
  interface Window {
    ...
    /** Server-injected flag: ``true`` when served by the Sidekick desktop
     * shell (``HERMES_DESKTOP=1``). The desktop webview swallows
     * ``target="_blank"`` link clicks, so {@link openExternal} routes
     * external URLs through ``/api/open-external`` instead. */
    __HERMES_DESKTOP__?: boolean;
  }
}
 ...
 *
 * In the Sidekick desktop shell the dashboard runs inside a single webview
 * that swallows ``target="_blank"`` link clicks, so a plain anchor opens
 * nothing. When the server-injected ``window.__HERMES_DESKTOP__`` flag is set
 * we POST the URL to the desktop-only ``/api/open-external`` bridge, which
 * hands it to the host OS default browser. In a normal browser we open a real
 * new tab. Any desktop-bridge failure also falls back to ``window.open`` so
 * the link is never completely dead.
 */
export async function openExternal(url: string): Promise<void> {
  if (window.__HERMES_DESKTOP__) {
    try {
      await fetchJSON<{ ok: boolean }>("/api/open-external", {
        method: "POST",
      ...
      // Bridge unavailable or refused - fall through to window.open so the
      // click still does something.
    }
  }
...
export type OpenRouterInfo = {
  ...
  council_default_models: string[];
};

export function getOpenRouterInfo(): Promise<OpenRouterInfo> {
  return fetchJSON<OpenRouterInfo>("/api/openrouter/info");
}
...
export function getLocalModels(): Promise<LocalModelsInfo> {
  return fetchJSON<LocalModelsInfo>("/api/local/models");
}

export function startLocalModelDownload(id: string): Promise<{ ok: boolean }> {
...
export function deleteLocalModel(id: string): Promise<{ ok: boolean }> {
  return fetchJSON<{ ok: boolean }>(
    `/api/local/models/${encodeURIComponent(id)}`,
    { method: "DELETE" },
  );
}
