...
import { submitStudioJob, submitVideoFromImage, type StudioJob } from "@/lib/studio-api";
...
type Props = {
  disabled: boolean;
  onComplete: (model: string, prompt: string, mode: "t2v" | "i2v", job: StudioJob, requestId: string) => void;
};

export function VideoStudio({ disabled, onComplete }: Props) {
  ...
  const submitMeta = useRef<{ model: string; prompt: string; mode: "t2v" | "i2v" } | null>(null);
  ...
  function onGenerate() {
    ...
    void run(() =>
      reference
        ? submitVideoFromImage(model, prompt.trim(), reference, values)
        : submitStudioJob("video", model, prompt.trim(), values),
    );
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      <ReferenceUpload
        ...
        onError={setError}
        label="Reference image (animate from image)"
      />

      <label className="flex flex-col gap-1 text-sm">
        <span className="text-text-tertiary">Model {reference ? "(image to video)" : "(text to video)"}</span>
