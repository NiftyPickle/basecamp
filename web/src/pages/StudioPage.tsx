...
import { loadHistory, appendHistory, clearHistory, type HistoryEntry } from "@/lib/studio-history";
...
export default function StudioPage() {
  ...
  return (
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-4 p-6" data-tour="studio">
      ...
      <div className="flex gap-6">
        <Card className="flex-1">
          {/* The route outlet provides no height or scroll context, so the page
              self-manages scroll like ConfigPage/ModelsPage do. Both studios stay
              mounted (visibility via `hidden`) so an in-flight job's poll chain
              survives tab switches - same keep-mounted pattern as the chat host. */}
          <CardContent className="max-h-[calc(100vh-260px)] overflow-y-auto p-5">
            ...
            <div className={tab === "templates" ? "" : "hidden"}>
              <TemplatesStudio disabled={disabled} onComplete={(label, media, job, id) => record(label, label, "effect", media, job, id)} />
            </div>
            ...
            <div className={tab === "marketing" ? "" : "hidden"}>
              <MarketingStudio disabled={disabled} onComplete={(motion, prompt, job, id) => record(motion, prompt, "marketing", "video", job, id)} />
            </div>
            ...
            <div className={tab === "workflows" ? "" : "hidden"}>
              <WorkflowsStudio
                disabled={disabled}
                onComplete={(workflow, job, id) => record(workflow, "", "t2i", "image", job, id)}
              />
            </div>
          </CardContent>
        </Card>
