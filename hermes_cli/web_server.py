...
# ---------------------------------------------------------------------------
# Studio (MUAPI media generation proxy) routes. Attached here so they inherit
# the /api/ auth gate defined above.
# ---------------------------------------------------------------------------
...
# Desktop-shell-only: open external links in the host OS browser. The desktop
# webview swallows ``target="_blank"`` clicks, so anchors to outside sites
# (e.g. the OpenRouter key page) would otherwise open nothing. Never mounted
# for a publicly-served dashboard - opening a browser on the server host is
# wrong off a local desktop process.
if os.getenv("HERMES_DESKTOP") == "1":
    from hermes_cli.desktop_bridge import register_desktop_bridge_routes

    register_desktop_bridge_routes(app)


# ---------------------------------------------------------------------------
# Chat-group (conversation library) routes. Auth-gated via /api/ like studio.
# ---------------------------------------------------------------------------
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    try:
        ...
        # The desktop bundle stages CPython + web_dist only — no Node/ink
        # `ui-tui`. _make_tui_argv does Path ops on the absent dir and
        # raises FileNotFoundError *before* its sys.exit, so the SystemExit
        # handler above never sees it. Degrade to a generic message (no raw
        # path) and close cleanly instead of a 500. The /sidekick chat
        # surface (in-process over /api/ws) is unaffected.
        _log.info("pty refused: terminal chat backend unavailable peer=%s", peer)
        await ws.send_text(
            "\r\n\x1b[31mChat unavailable: the embedded terminal isn't "
            "bundled in this build.\x1b[0m\r\n"
            "\x1b[33mUse the Chat tab in the sidebar instead.\x1b[0m\r\n"
        )
        await ws.close(code=1011)
        return
...
@app.websocket("/api/council/ws")
async def council_ws(ws: WebSocket) -> None:
    if not _DASHBOARD_EMBEDDED_CHAT_ENABLED:
        await ws.close(code=4403)
        return
    ...
    from hermes_cli.council.ws import handle_council_ws

    await handle_council_ws(ws)


@app.get("/api/openrouter/info")
async def openrouter_info() -> dict:
    from hermes_cli.council.info import build_openrouter_info

    return build_openrouter_info()
...
def mount_spa(application: FastAPI):
    ...
    def _serve_index(prefix: str = ""):
        ...
        # Signals the SPA it is running inside the Sidekick desktop shell, so
        # external links route through /api/open-external (the webview swallows
        # ``target="_blank"``) instead of window.open.
        desktop_js = "true" if os.getenv("HERMES_DESKTOP") == "1" else "false"
        if gated:
            bootstrap_script = (
                f"<script>"
                ...
                f"window.__HERMES_DESKTOP__={desktop_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
